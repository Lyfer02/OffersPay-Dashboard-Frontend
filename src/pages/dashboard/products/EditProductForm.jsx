    ...
    useEffect(() => {
        const fetchAllData = async () => {
            try {
                ...
                const [categoriesRes, storesRes, brandsRes] = await Promise.all([
                ...
                // Only fetch product data if ID exists
                if (id) {
                    ...
                    if (productRes.status === 200 && productRes.data) {
                        ...
                        // Update form data with fetched product data
                        setFormData(prevData => ({
                            ...prevData,
                            name: productData.name || '',
                            brand: productData.brand?._id || productData.brand || '',
                            store: productData.store?._id || productData.store || '',
                            category: productData.category?._id || productData.category || '',
                            startingPrice: productData.startingPrice || '',
                            sellingPrice: productData.sellingPrice || '',
                            earningRate: productData.earningRate || '',
                            description: productData.description || '',
                            dealsDetails: productData.dealsDetails || '',
                            productUrl: productData.productUrl || '',
                            trackingUrl: productData.trackingUrl || '',
                            inStock: productData.inStock !== undefined ? productData.inStock : true,
                            status: productData.status || 'active',
                            isUpcoming: productData.isUpcoming || false,
                        ...
                        // Set available offers if they exist
                        if (productData.availableOffers && Array.isArray(productData.availableOffers)) {
                            setOffers(productData.availableOffers.length > 0 ? productData.availableOffers : ['']);
                        }
                        ...
                        if (productData.logo) {
                            setSelectedLogo({
                                file: null,
                                preview: getImageUrl(productData.logo),
                                name: 'existing-logo',
                                isExisting: true
                            });
                        }

                        // Set slider images if they exist
                        if (productData.sliderImages && Array.isArray(productData.sliderImages)) {
                            const existingImages = productData.sliderImages.map((img, index) => ({
                                file: null,
                                preview: getImageUrl(img),
                                name: `existing-slider-${index}`,
                                isExisting: true
                            }));
