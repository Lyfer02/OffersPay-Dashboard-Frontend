...
const EditIp = () => {
  ...
  const fetchIpDetails = async () => {
    setIsLoadingIp(true);
    try {
      const res = await IpService.getById(id);
      ...
      setFormData({
        network: ipData.network?._id || '',
        ip: ipData.ip || '',
        status: ipData.status || 'draft',
      });
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await IpService.updateById(id, {
      ...
      console.log('Updated IP Whitelist:', res);
      handleClose();
    } catch (error) {
      console.error('error', error);
      toast.error(
        error?.response?.data?.message || 'Failed to update IP whitelist'
      );
    }
  };
  ...
  if (isLoadingIp) {
    return (
      <div className="flex items-center justify-center h-screen">
        <Spinner className="h-8 w-8" /> <span className="ml-2">Loading IP details...</span>
      </div>
    );
  }

  return (
    <Card className="min-h-screen max-w-4xl mx-auto my-10 rounded-2xl">
      <CardHeader
        variant="gradient"
        color="blue"
      ...
      >
        <div>
          ...
          <Typography variant="small" color="white" className="opacity-80">
            Update the IP whitelist information
          </Typography>
        </div>
        <Button
          variant="outlined"
